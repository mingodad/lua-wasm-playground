--From: https://github.com/eatkins/tree-sitter-ebnf-generator/blob/b25508d4ed3c6f9926acce70b2a1d0847d45363a/src/lua/parse_grammar.lua
-- Adapted to run on https://mingodad.github.io/lua-wasm-playground/
--#!/usr/bin/env lua
...
local IN_REGEX_FLAGS = IN_REGEX + 1
local IN_MINI_REGEX = IN_REGEX_FLAGS + 1
...
local function parse_body(id, string)
  ...
  for _, b in ipairs(bytes) do
    ...
    if state == IN_REGEX_FLAGS then
      local chb = char(b)
      if not chb:match("[gimsuy]") then
        add_literal(node, current_word)
        current_word = nil
        table.remove(state_stack)
    ...
    elseif b == FORWARD_SLASH and state < _IN_CHAR_GROUPING then
      ...
      current_word = { "/" }
    elseif b == FORWARD_SLASH and state == IN_REGEX and state ~= IN_MINI_REGEX and (last_byte ~= BACK_SLASH or last_byte == prev_last_byte) then
      table.insert(current_word, "/")
      table.remove(state_stack)
      table.insert(state_stack, IN_REGEX_FLAGS)
  ...
  if state_stack[#state_stack] == IN_REGEX_FLAGS then
    add_literal(node_stack[#node_stack], current_word)
    current_word = nil
    table.remove(state_stack)
  end
...
for _, v in ipairs(tokens) do
  ...
  local index = file_name .. ":" .. line_num
...
local content = [[
// This file was autogenerated by parse_grammar.lua from ]] .. file_name .. [[.
